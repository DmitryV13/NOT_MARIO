...
#include "ImageButton.h"
	...
	ImageButton::ImageButton(float x, float y, float width, float height, short text_size, sf::Font* font_
		...
		float img_max_height = (shape.getSize().x * 80) / 100;
		//float it = 80;
		...
		if (text.getGlobalBounds().width == 0) {
			...
			btn_image->setPosition(
				shape.getPosition().x - shape.getOutlineThickness() - text.getLocalBounds().left + (shape.getGlobalBounds().width - text.getLocalBounds().width - btn_image->getGlobalBounds().width - btn_image->getGlobalBounds().width / 2) / 2,
		...
		text.setPosition(
			btn_image->getPosition().x + btn_image->getGlobalBounds().width / 2,
			shape.getPosition().y - text.getLocalBounds().top + (shape.getGlobalBounds().height - text.getLocalBounds().height) / 2
		);

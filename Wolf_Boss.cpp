		...
		case Boss_STATE::JUMPING:
			{
				...
				walk(moving);
				if (std::abs(player_->getPosition().x - get_position().x)<4*60)
				{
...
void Wolf_Boss::update_physics()
{
	...
	if (jump_tile)
	{
		...
		displacement.x += moving *4 * acceleration;
		//jump deceleratin
		jump_velocity *= 0.96;
	}
	// deceleration
